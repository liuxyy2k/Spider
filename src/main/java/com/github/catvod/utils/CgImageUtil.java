...
import java.util.Base64;  // Use the standard Java Base64
...
public class CgImageUtil {
    ...
    private static String aesDecrypt(String word) {
        try {
            ...
            byte[] srcBytes = Base64.getDecoder().decode(word);  // Use Java Base64 decoder
            ...
            return Base64.getEncoder().encodeToString(decryptedBytes);  // Use Java Base64 encoder
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
        ...
        if (isCdnImg(bgUrl)) {
            try {
                ...
                if (body != null) {
                    ...
                    String base64Str = Base64.getEncoder().encodeToString(imageBytes);  // Java Base64 encoding
